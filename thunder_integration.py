...
class ThunderClientIntegration:
    ...
    def update_statistics(self):
        ...
        for collection_name, collection_data in self.collections.items():
            ...
            if isinstance(collection_data, dict):
                # Format Postman avec 'item'
                if 'item' in collection_data:
                    requests_count = len(collection_data.get('item', []))
                # Format Thunder Client avec 'requests'  
                elif 'requests' in collection_data:
                    requests_count = len(collection_data.get('requests', []))
                # Format direct
                else:
                    requests_count = len(collection_data.get('entries', []))
    ...
    def get_collection_requests(self, collection_name: str) -> List[str]:
        ...
        if isinstance(collection_data, dict):
            # Format Postman avec 'item'
            if 'item' in collection_data:
                return [req.get('name', f'Request_{i}') for i, req in enumerate(collection_data.get('item', []))]
            # Format Thunder Client avec 'requests'
            elif 'requests' in collection_data:
                return [req.get('name', f'Request_{i}') for i, req in enumerate(collection_data.get('requests', []))]
            # Format direct
            else:
                return [req.get('name', f'Request_{i}') for i, req in enumerate(collection_data.get('entries', []))]
        elif isinstance(collection_data, list):
    ...
    def get_request_details(self, collection_name: str, request_name: str) -> Optional[Dict]:
        ...
        if isinstance(collection_data, dict):
            # Format Postman avec 'item'
            if 'item' in collection_data:
                requests_list = collection_data.get('item', [])
            # Format Thunder Client avec 'requests'
            elif 'requests' in collection_data:
                requests_list = collection_data.get('requests', [])
            # Format direct
            else:
                requests_list = collection_data.get('entries', [])
        elif isinstance(collection_data, list):
            requests_list = collection_data
        
        for request in requests_list:
            if request.get('name') == request_name:
                # Adaptation pour format Postman
                if 'request' in request:
                    # Format Postman : { "name": "...", "request": { "method": "POST", "body": {...} } }
                    request_details = request['request']
                    # Extraire le body si c'est du JSON
                    if 'body' in request_details and 'raw' in request_details['body']:
                        try:
                            import json
                            body_content = json.loads(request_details['body']['raw'])
                            return {
