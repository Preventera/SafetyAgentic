...
Version FINALE CORRIGÉE avec Mémoire IA
"""
...
try:
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("🧠 Mémoire IA Enhanced")
    ...
    user_company = st.sidebar.text_input("🏢 Nom Entreprise", placeholder="Ex: Construction ABC", key="memory_company")
    use_memory = st.sidebar.checkbox("🧠 Activer Mémoire IA", value=True, key="memory_toggle")
    
    if user_company and st.sidebar.button("📚 Historique", key="memory_history"):
        memories = get_agent_context("A1", user_company, "analyse", limit=3)
        if memories:
            st.sidebar.success(f"📚 {len(memories)} analyses")
            for i, memory in enumerate(memories):
                metadata = memory.get("metadata", {})
                st.sidebar.write(f"#{i+1}: {metadata.get('score', 0)}/100")
        else:
            st.sidebar.info("📭 Aucun historique")
    
    if 'memory_config' not in st.session_state:
        st.session_state.memory_config = {"user_company": user_company, "use_memory": use_memory}
    
except Exception as e:
    st.sidebar.error(f"❌ Erreur: {e}")
    st.session_state.memory_config = {"memory_available": False}
            ...
            if st.button("🚀 Lancer l'Analyse", type="primary", use_container_width=True):
                if user_query:
                    with st.spinner("🔄 Analyse en cours..."):
                        ...
                        # Intégration Mémoire IA - Agent A1 Enhanced
                        try:
                            import sys
                            sys.path.append("src")
                            from safegraph_memory_integration import enhance_analysis_with_memory, MEMORY_AVAILABLE
                            
                            if MEMORY_AVAILABLE:
                                result = enhance_analysis_with_memory(user_query, secteur, debug_mode)
                                st.info("🧠 Analyse enrichie par la Mémoire IA - Agent A1 Enhanced")
                            else:
                                result = run_safegraph_analysis(user_query, secteur, debug_mode)
                                st.warning("📊 Analyse standard SafeGraph")
                                
                        except Exception as e:
                            st.warning(f"⚠️ Mémoire IA indisponible, analyse standard: {e}")
                            result = run_safegraph_analysis(user_query, secteur, debug_mode)
...
with tab4:
    ...
    if 'analysis_result' in st.session_state:
        ...
        # DEBUG : Afficher les nouvelles données mémoire IA
        st.write("🔍 **DEBUG - Clés disponibles:**", list(result.keys()))
        
        # Afficher les nouvelles données mémoire
        if result.get('memory_insights'):
            st.success("🧠 **Analyse enrichie par la Mémoire IA !**")
            st.write("📊 Memory Context:", result.get('memory_context', {}))
        
        if result.get('enhanced_by_memory'):
            st.info("✨ **Enhanced by Memory:** True")
